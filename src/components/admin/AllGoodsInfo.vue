<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>所有记录</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <el-table :data="searchData" border :stripe="true">
        ...
        <el-table-column prop="goodsInCount" label="数量"></el-table-column>
...
  },
    ...
    deleteData(scope) {      
      ...
      this.inintData()
      let comValue = this.$qs.stringify(scope.row.goodsId)
      //console.log("出库的货物编码:",scope.row.goodsId)
                                                   //返回用户姓名，后端根据userName进行相关处理    将该商品从商品展示的数据库中删除并保存到出库记录数据库中
       this.$http.post("/removeCommodity/outbound", comValue).then(res => {
          this.$message({
            message: "操作成功",
            type: "success"
          });
